...
*** Makefile以外のファイルを読み込ませたい。 [#q1e1905d]
...
*** Makefile内で-(ハイフン)を付加していなくても、コマンドがエラー時は継続させたい [#df38da0c]
-iオプションで全てのMakefile内のコマンドに-(ハイフン)が付与されているようにみせかけることができます。
処理がエラーとなっても継続したい場合に便利です。
   $ make -i 

*** Makefile内で@を付加していなくても、コマンド出力をしないようにしたい。 [#wd949719]
-sオプションで全てのMakefile内のコマンドに@が付与されているようにみせかけることができます。
   $ make -s 

*** デバッグ情報を出力したい [#m34dd139]
--debugオプションを付加すればよい。
   $ make --debug 

*** makeの一般的なオプションについて [#h5374b11]
	-p 
...
*** (2)画面に実行した結果を非表示にしたい。 [#yb72ed84]
Makefileに記述したコマンドは画面上に表示されてしまうので、非表示にしたい
場合があるかと思います。
このような場合は「@」をコマンドの先頭につけます。(1)の例で「touch testfile」
を画面上に表示させないようにします。
 clean: 
 	@touch testfile

*** (3)ターゲットをコマンドの一部に利用したい [#z5113855]
 touch:
 	$@ testfile
上記では「make touch」とすると、「touch testfile」が実行されます。

*** (4)途中で実行エラーとなっても、最後まで処理を行わせたい。 [#h2d5f89d]
 touch:
 	@echo "test1"
 	-@rm zzzzz
 	@echo "test2"
...
例えば、Makefile2として、makeするディレクトリから1つ階層が下のincludeというディレクトリ
中に設置した場合には以下の様にします。
...
*** (6)入力されたターゲットやターゲットの一部を利用したい。 [#zb38bedb]
 test-%:
...
*** (7)あるファイルが存在しないとターゲットを実行させない。(依存) [#zb3f1cf0]
...
*** (9)変数の一部を変更したい。(拡張子の一部を変更する) [#yec35778]
...
*** (10) for文等で利用する変数「$i」を解釈させたい。 [#w0779c5c]
