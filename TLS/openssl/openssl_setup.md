# 概要
opensslコマンドを主体にして次のようなことについてまとめる
- opensslのビルド
- 秘密鍵・公開鍵生成
- TLSセットアップと確認

証明書確認などはopenssl_cert.mdなどと若干重複有り

# 詳細

### opensslビルド時に覚えておくことについて
opensslはビルド時のソフトウェアが大きく影響する。gccバージョンとgasバージョンによってIntel暗号化処理機能が制限されている。
- https://github.com/openssl/openssl/blob/OpenSSL_1_0_2-stable/crypto/modes/asm/aesni-gcm-x86_64.pl#L44-L47

gas-2.22以上ならAVX2を利用、2.21〜2.20はAVXのみ、それ以下はAVXサポート無しといった扱いとなっているようだ。

確認方法。以下だと2.25なのでAVX2対応している。gccは4.9以上であればよい。
```
$ gcc -v
